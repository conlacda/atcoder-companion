addInOutColumnToResultTable = (resultTable, inOrOut) => {
    ...
    rows.each(function (index, row) {
        const tcfile = $(this).find('td:first').text().split('.')[0];
        // add copy button
        ...
        copyIcon.attr('id', `copy-${inOrOut}-${index}`);
        ...
        copiedIcon.attr('id', `copied-${inOrOut}-${index}`);
        ...
        downloadIcon.attr('id', `download-${inOrOut}-${index}`);
        downloadIcon.on("click", async () => {
            const fileName = `${inOrOut}-${$(this).find('td:first').text()}`;
